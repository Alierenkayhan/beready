...
namespace Overrides
{
    ...
    public class XROriginOverride : XROrigin
    {
        private Camera m_Camera;
        
        public GameObject cameraParent;
        public float HorizontalMovementMultiplier = 1.0f;
        private static readonly Vector3 HorizontalVector = new Vector3(1,0,1);
        ...
        /// </summary>
        /// <remarks>
        /// You can add a <see cref="TrackedPoseDriver"/> component to the <see cref="Camera"/> 
        ...
        public GameObject CameraParent
        {
            ...
            {
                cameraParent = value;
                m_Camera = value.transform.GetChild(0).GetComponent<Camera>();
            }
        }
        ...
        /// </summary>
        public Vector3 OriginInCameraSpacePos => cameraParent.transform.InverseTransformPoint(m_OriginBaseGameObject.transform.position);

        /// <summary>
        /// (Read Only) The camera's local position in origin space.
        /// </summary>
        public Vector3 CameraInOriginSpacePos => m_OriginBaseGameObject.transform.InverseTransformPoint(cameraParent.transform.position);
        ...
        public bool RotateAroundCameraPosition(Vector3 vector, float angleDegrees)
        {
            ...
            m_OriginBaseGameObject.transform.RotateAround(cameraParent.transform.position, vector, angleDegrees);
        ...
        public bool MatchOriginUpCameraForward(Vector3 destinationUp, Vector3 destinationForward)
        {
            if (cameraParent != null && MatchOriginUp(destinationUp))
            {
                ...
                var projectedCamForward = Vector3.ProjectOnPlane(cameraParent.transform.forward, destinationUp).normalized;
        ...
        public bool MoveCameraToWorldLocation(Vector3 desiredWorldLocation)
        {
            ...
            var rot = Matrix4x4.Rotate(cameraParent.transform.rotation);
        ...
        protected void Awake()
        {
            ...
            if (m_Camera == null || cameraParent == null)
            {
            ...
            if (cameraParent)
            {
                ...
                var trackedPoseDriver = cameraParent.GetComponent<UnityEngine.InputSystem.XR.TrackedPoseDriver>();
                var trackedPoseDriverOld = cameraParent.GetComponent<UnityEngine.SpatialTracking.TrackedPoseDriver>();
                if (trackedPoseDriver == null && trackedPoseDriverOld == null)
                {
                    Debug.LogWarning(
                        $"Camera \"{cameraParent.name}\" does not use a Tracked Pose Driver (Input System), " +
                ...
                {
                    Debug.LogWarning(
                        $"Camera \"{cameraParent.name}\" does not use a Tracked Pose Driver, and com.unity.xr.legacyinputhelpers is installed. " +
...
#elif !INCLUDE_INPUT_SYSTEM && !INCLUDE_LEGACY_INPUT_HELPERS
                Debug.LogWarning(
                    $"Camera \"{cameraParent.name}\" does not use a Tracked Pose Driver and com.unity.inputsystem is not installed, " +
                    "so its transform will not be updated by an XR device.  In order for this to be " +
        ...
        Pose GetCameraOriginPose()
        {
            ...
            var parent = cameraParent.transform.parent;
        ...
        private void Update()
        {
            if (cameraParent)
            {
                ...
                TrackablesParent.position = pose.position.Multiply(HorizontalVector * HorizontalMovementMultiplier);
